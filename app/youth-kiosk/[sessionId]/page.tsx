...
type Step = "phone" | "returning" | "new-form" | "confirm";

type SessionInfo = { id: string; name: string; date: string; ministry_type: string; status: string };
...
export default function YouthKioskPage() {
  ...
  useEffect(() => {
    fetch(`/api/youth-checkin/sessions?id=${sessionId}`)
      .then(r => r.json())
      .then(d => {
  ...
  async function handlePhoneLookup() {
    ...
    setLooking(false);
    if (data.success) {
      // Returning student was checked in immediately
  ...
  if (step === "phone") return (
    <div style={containerStyle}>
      <div style={headerStyle}>
        ...
        <h1 style={{ margin: "8px 0 0", fontSize: 28, fontWeight: 900, color: "white" }}>Youth Check-In</h1>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 16 }}>
          {[
            ...
            <div key={label}>
              ...
              <input type={type} value={value} onChange={e => set(e.target.value)} style={{ width: "100%", fontSize: 20, padding: "14px 16px", borderRadius: 12, border: "2px solid #e5e7eb", boxSizing: "border-box" as const }} />
            </div>
          ))}
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 16 }}>
