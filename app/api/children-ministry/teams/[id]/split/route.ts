...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  await admin.from('children_ministry_points').insert([
    { church_id: churchId, season_id: team.season_id, team_id: teamId, points: SPLIT_BONUS, category: 'split_bonus', awarded_by: user.id, note: `Team split bonus — ${newTeamName.trim()} created` },
    { church_id: churchId, season_id: team.season_id, team_id: newTeam.id, points: SPLIT_BONUS, category: 'split_bonus', awarded_by: user.id, note: `Team split bonus — new team from ${team.name}` },
  ]);
