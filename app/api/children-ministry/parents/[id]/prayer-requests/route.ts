...
export async function GET(
  ...
  const { data, error } = await adminClient()
    ...
    .select('id, request_text, status, created_by, created_by_name, created_at, updated_at, answered_at')
    .eq('family_id', id)
...
export async function POST(
  ...
  if (!requestText) return Response.json({ error: 'requestText is required' }, { status: 400 });
