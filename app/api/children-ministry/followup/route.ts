...
export async function GET(request: NextRequest) {
  ...
  if (phones.length) {
    ...
    const { data: visitorFamilies } = await admin
      ...
      .select('parent1_phone, parent1_email, parent2_phone, parent2_email')
      .eq('church_id', auth.churchId)
      .or(`parent1_phone.in.(${phoneList}),parent2_phone.in.(${phoneList})`);
    for (const vf of visitorFamilies ?? []) {
      if (vf.parent1_phone && vf.parent1_email) emailMap[vf.parent1_phone] = vf.parent1_email;
      if (vf.parent2_phone && vf.parent2_email) emailMap[vf.parent2_phone] = vf.parent2_email;
    }
  }
...
export async function POST(request: NextRequest) {
  ...
  const {
    ...
    childNames, followUpType, subject, bodyText,
