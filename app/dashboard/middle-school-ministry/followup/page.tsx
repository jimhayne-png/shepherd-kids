...
export default function MiddleSchoolFollowUpPage() {
  ...
  useEffect(() => {
    async function init() {
      ...
      const results = await Promise.all(
        toFetch.map(s =>
          fetch(`/api/middle-school-ministry/students/${s.id}/followup`, {
            headers: { Authorization: `Bearer ${t}` },
  ...
  async function logTouch(studentId: string, touch: 1 | 2 | 3, completed: boolean) {
    ...
    setSavingTouch(key);
    await fetch(`/api/middle-school-ministry/students/${studentId}/followup`, {
      method: "PATCH",
