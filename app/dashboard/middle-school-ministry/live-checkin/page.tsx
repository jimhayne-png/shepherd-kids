...
export default function MSLiveCheckinPage() {
  ...
  const fetchRecords = useCallback(async (tok: string, sessionId: string) => {
    const res = await fetch(`/api/middle-school-ministry/live?sessionId=${sessionId}`, {
  ...
  useEffect(() => {
    supabase.auth.getSession().then(async ({ data: { session } }) => {
      ...
      const res = await fetch("/api/middle-school-ministry/checkin-sessions", {
  ...
  return (
    <AppShell navItems={navItems}>
      ...
      <div style={{ background: "linear-gradient(135deg, #c2570a 0%, #F28C28 100%)", padding: "28px 32px" }}>
        <Link href="/dashboard/ministry/middle-school" style={{ color: "rgba(255,255,255,0.8)", fontSize: 14, textDecoration: "none", display: "inline-block", marginBottom: 6 }}>
          ← Middle School
        </Link>
      ...
      <div style={{ padding: "32px", maxWidth: 900, margin: "0 auto" }}>
        {loading ? (
          ...
          <div style={{ backgroundColor: "#fffbeb", border: "1px solid #fde68a", borderRadius: 12, padding: "18px 22px", color: "#92400e", fontSize: 15 }}>
            ...
            <Link href="/dashboard/middle-school-ministry/checkin-setup" style={{ color: "#92400e", fontWeight: 700, textDecoration: "underline" }}>
              Check-In Setup
            </Link>.
          </div>
        ) : (
