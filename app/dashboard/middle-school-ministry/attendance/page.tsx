...
export default function MiddleSchoolAttendancePage() {
  ...
  useEffect(() => {
    async function init() {
      ...
      setToken(t);
      const res = await fetch("/api/middle-school-ministry/attendance", {
  ...
  useEffect(() => {
    ...
    async function loadRecords() {
      ...
      const res = await fetch(`/api/middle-school-ministry/attendance?sessionId=${selectedSessionId}`, {
  ...
  function exportCsv() {
    ...
    a.href = url;
    a.download = `ms-attendance-${selectedSessionId}.csv`;
    a.click();
  ...
  return (
    <AppShell navItems={navItems}>
      ...
      <div className="px-8 py-10" style={{ background: `linear-gradient(135deg, #c2570a 0%, ${ACCENT} 100%)` }}>
        <Link href="/dashboard/ministry/middle-school" className="text-orange-200 text-xs mb-1 block hover:text-white">
          ← Middle School
        </Link>
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-2xl shadow border border-gray-100 p-12 text-center">
            ...
            <p className="text-gray-400 text-sm">
              Create one in{" "}
              <Link href="/dashboard/middle-school-ministry/checkin-setup" className="underline" style={{ color: ACCENT }}>
                Check-In Setup
              </Link>
              .
            </p>
          ...
          <>
            ...
            <div className="grid grid-cols-3 gap-4 mb-6">
              {[
                ...
                { label: "New Visitors", value: stats.newVisitors, color: ACCENT, bg: "#fff7ed" },
                { label: "Returning", value: stats.returning, color: "#10b981", bg: "#f0fdf4" },
              ].map(card => (
