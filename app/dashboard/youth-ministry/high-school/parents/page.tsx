...
export default function HighSchoolParentsPage() {
  ...
  const filtered = useMemo(() => {
    const list = parents.filter(p => p.youth_students?.grade != null && HS_GRADES.has(p.youth_students.grade));
  ...
  return (
    <AppShell navItems={navItems}>
      <div className="px-8 py-10" style={{ background: `linear-gradient(135deg, #c2570a 0%, ${ACCENT} 100%)` }}>
        ...
        <h1 className="text-3xl font-bold text-white" style={{ fontFamily: "Georgia, serif" }}>Senior High Parents</h1>
      ...
      <div className="px-8 py-8 bg-gray-50 min-h-screen">
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-2xl shadow border border-gray-100 p-12 text-center">
            <p className="text-gray-400">{search ? "No parents match your search." : "No senior high parents on file yet."}</p>
          </div>
        ) : (
