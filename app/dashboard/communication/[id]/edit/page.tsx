...
export default function EditPostPage({
  ...
  const [postType, setPostType] = useState<"church_wide" | "department">("church_wide");
  ...
  useEffect(() => {
    async function init() {
      ...
      const [postRes, deptsRes] = await Promise.all([
        fetch(`/api/communication/${id}`, { headers: { Authorization: `Bearer ${session.access_token}` } }),
        fetch("/api/departments", { headers: { Authorization: `Bearer ${session.access_token}` } }),
      ]);

      if (!postRes.ok) { router.replace("/dashboard/communication"); return; }
      ...
      setNotifyEmail(post.notify_email);
      setDepartments(deptsData.departments ?? []);

      if (post.department_id) {
        setPostType("department");
  ...
  async function handleSubmit(saveStatus: "draft" | "published") {
    ...
    const res = await fetch(`/api/communication/${id}`, {
      ...
      body: JSON.stringify({
        ...
        status: scheduled ? "scheduled" : saveStatus,
    ...
    <AppShell navItems={navItems}>
      <div style={{ padding: "32px", maxWidth: "680px" }}>
        ...
        <div style={{ marginBottom: "24px" }}>
          ...
          <div style={{ display: "flex", border: "2px solid #e5e7eb", borderRadius: "10px", overflow: "hidden", width: "fit-content" }}>
            {([
        ...
        {postType === "department" && (
          <div style={{ marginBottom: "20px" }}>
            <label style={{ fontFamily: "Georgia, serif", fontSize: "14px", color: "#374151", display: "block", marginBottom: "8px" }}>Department *</label>
            <select
              value={departmentId}
        ...
        <div style={{ background: "#f9fafb", borderRadius: "12px", padding: "20px", marginBottom: "24px", display: "flex", flexDirection: "column", gap: "16px" }}>
          ...
          {scheduled && (
            <input
              ...
              style={{ padding: "10px 14px", borderRadius: "8px", border: "2px solid #e5e7eb", fontFamily: "Georgia, serif", fontSize: "14px", outline: "none" }}
            />
          )}
          <button
            type="button"
          ...
          >
            ...
            <span style={{ fontFamily: "Georgia, serif", fontSize: "14px", color: "#374151" }}>Notify via email {notifyEmail ? "(on)" : "(off)"}</span>
          </button>
        </div>
