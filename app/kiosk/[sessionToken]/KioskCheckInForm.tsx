...
export default function KioskCheckInForm({ sessionToken, serviceName, churchName, rooms }: Props) {
  ...
  // Called from welcome screen — phone lookup happens here
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data.found && data.children?.length > 0) {
          // Returning family — pre-fill and skip to children
          ...
          // New family — carry phone to parent form
          setParentPhone(welcomePhone);
          setReturning(false);
          setStep("parent");
        }
  ...
  // ── welcome (phone lookup lives here now) ─────────────────────────────────

  if (step === "welcome") {
    const canContinue = welcomePhone.replace(/\D/g, "").length >= 7;
    return (
      <div className="min-h-screen flex flex-col items-center justify-center p-8" style={{ backgroundColor: BG }}>
        <div className="w-full max-w-lg text-center">
          ...
          <div className="text-left mb-6">
            ...
            <input
              ...
              onKeyDown={(e) => { if (e.key === "Enter" && canContinue) handleWelcomeContinue(); }}
      ...
      <div className="min-h-screen" style={{ backgroundColor: BG }}>
        <div className="flex flex-col items-center p-6">
          <div className="w-full max-w-lg">
            ...
            {/* Parent summary card for returning families with Edit button */}
            {returning && (
              <div
              ...
              >
                <div>
                  <p className="text-green-400 text-xs font-bold uppercase tracking-widest mb-1">Checking in as</p>
                  <p className="text-white font-semibold">{parentFirstName} {parentLastName}</p>
                  <p className="text-green-500 text-sm">{parentPhone}</p>
                </div>
                <button
                  onClick={() => setStep("parent")}
                  className="text-sm font-bold px-4 py-2 rounded-xl"
                  style={{ backgroundColor: "#1a4d1a", color: GREEN, border: `1px solid ${GREEN}` }}
                >
                  Edit
                </button>
              </div>
            ...
            {/* Only show back button for new families — returning families use the Edit button above */}
            {!returning && (
              <button
                onClick={() => setStep("parent")}
                className="w-full mt-4 py-3 rounded-2xl text-sm font-medium text-green-600"
  ...
  return (
    <div style={LABEL_STYLE}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <span style={{ fontSize: 10, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.08em", backgroundColor: "#000", color: "#fff", padding: "1px 6px", borderRadius: 3 }}>
          Child Check-In
        </span>
        {label.roomName && (
          <span style={{ fontSize: 11, fontWeight: 700, border: "1.5px solid #000", padding: "1px 8px", borderRadius: 3 }}>
            {label.roomName}
          </span>
        )}
      </div>
      <div style={{ fontSize: 26, fontWeight: 900, lineHeight: 1.1, margin: "4px 0 0" }}>{label.childName}</div>
      <div style={{ fontSize: 11, color: "#333", marginTop: 2 }}>
        Parent: {label.parentName}{label.parentPhone ? ` · ${label.parentPhone}` : ""}
      </div>
      {(label.allergies || label.medicalNotes || label.specialInstructions) && (
        <div style={{ marginTop: 4 }}>
          {label.allergies && (
            <div style={{ fontSize: 10, fontWeight: 700, color: "#fff", backgroundColor: "#dc2626", padding: "2px 6px", borderRadius: 3, display: "inline-block", marginBottom: 2 }}>
              ⚠ ALLERGY: {label.allergies}
            </div>
          )}
          {label.medicalNotes && <div style={{ fontSize: 10, color: "#333" }}><strong>Medical:</strong> {label.medicalNotes}</div>}
          {label.specialInstructions && <div style={{ fontSize: 10, color: "#333" }}><strong>Instr:</strong> {label.specialInstructions}</div>}
        </div>
      )}
      <div style={{ display: "flex", justifyContent: "flex-end", alignItems: "flex-end", marginTop: "auto" }}>
        <div>
          ...
          <div style={{ fontSize: 28, fontWeight: 900, fontFamily: "monospace", letterSpacing: "0.18em", lineHeight: 1 }}>{label.securityCode}</div>
        </div>
      </div>
    </div>
  );
...
function KioskParentLabel({ label }: { label: ImmediateLabel }) {
  return (
    <div style={LABEL_STYLE}>
      <div style={{ backgroundColor: "#000", color: "#fff", fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.1em", padding: "2px 8px", alignSelf: "flex-start", borderRadius: 3 }}>
        👪 Parent Pickup
      </div>
      <div style={{ fontSize: 22, fontWeight: 900, lineHeight: 1.1, marginTop: 6 }}>{label.parentName}</div>
      <div style={{ fontSize: 12, color: "#333", marginTop: 4 }}>{label.childName}</div>
      <div style={{ marginTop: "auto", borderTop: "1.5px solid #000", paddingTop: 6 }}>
        ...
        <div style={{ fontSize: 40, fontWeight: 900, fontFamily: "monospace", letterSpacing: "0.2em", lineHeight: 1 }}>{label.securityCode}</div>
      </div>
    </div>
  );
}
...
function ChildCard({
  index, child, rooms, showRemove, onChange, onRemove,
}: {
  index: number;
  child: ChildForm;
  rooms: Room[];
  ...
  function toggleAllergy(opt: string) {
    ...
    onChange({ ...child, allergies: next, allergyOther: isSelected && opt === "Other" ? "" : child.allergyOther });
  }
  ...
  return (
    <div className="rounded-2xl p-5 mb-4" style={{ backgroundColor: CARD, border: "1px solid #2d5a2d" }}>
      <div className="flex justify-between items-center mb-4">
        ...
        {showRemove && <button onClick={onRemove} className="text-red-400 text-sm font-bold">Remove</button>}
      </div>
      <div className="grid grid-cols-2 gap-3 mb-4">
        <div>
          <label className={labelCls}>First Name *</label>
          <input type="text" value={child.firstName} onChange={(e) => onChange({ ...child, firstName: e.target.value })} placeholder="First" className={inputCls} />
        </div>
        <div>
          <label className={labelCls}>Last Name *</label>
          <input type="text" value={child.lastName} onChange={(e) => onChange({ ...child, lastName: e.target.value })} placeholder="Last" className={inputCls} />
        </div>
      </div>
      <div className={`grid gap-3 mb-4 ${rooms.length > 0 ? "grid-cols-2" : "grid-cols-1"}`}>
        <div>
          <label className={labelCls}>Date of Birth *</label>
          <input type="date" value={child.dateOfBirth} max={new Date().toISOString().slice(0, 10)} min="2000-01-01" onChange={(e) => onChange({ ...child, dateOfBirth: e.target.value })} required className={inputCls} style={{ colorScheme: "dark" }} />
        </div>
        {rooms.length > 0 && (
          <div>
            <label className={labelCls}>Room</label>
            <select value={child.roomId} onChange={(e) => onChange({ ...child, roomId: e.target.value })} className={inputCls} style={{ appearance: "none" }}>
              <option value="" style={{ backgroundColor: BG }}>Select room</option>
              {rooms.map((r) => <option key={r.id} value={r.id} style={{ backgroundColor: BG }}>{r.name}</option>)}
            </select>
          </div>
        )}
      </div>
      <div className="mb-4">
        ...
        <div className="grid grid-cols-2 gap-2 mb-2">
          {ALLERGY_OPTIONS.map((opt) => {
            ...
            return (
              <button key={opt} type="button" onClick={() => toggleAllergy(opt)} className="text-left rounded-xl px-3 py-2 text-sm font-semibold transition-colors"
                style={{ backgroundColor: selected ? "#1a4d1a" : "rgba(255,255,255,0.05)", border: `1.5px solid ${selected ? GREEN : "#2d5a2d"}`, color: selected ? GREEN : "#6b9b6b" }}>
                {selected ? "✓ " : ""}{opt}
              </button>
            );
          })}
        ...
        {child.allergies.includes("Other") && (
          <input type="text" value={child.allergyOther} onChange={(e) => onChange({ ...child, allergyOther: e.target.value })} placeholder="Describe allergy" className={inputCls} style={{ fontSize: "1rem", padding: "12px 20px" }} />
        )}
      </div>
      <div className="mb-4">
        <label className={labelCls}>Special Instructions / Medical Notes (optional)</label>
        <textarea value={child.specialInstructions} onChange={(e) => onChange({ ...child, specialInstructions: e.target.value })} placeholder="e.g. carries EpiPen, bathroom reminder every hour" rows={2} className="w-full px-5 py-4 rounded-2xl bg-white/10 border border-green-800 text-white placeholder-green-700 focus:outline-none focus:border-green-400 resize-none text-base" />
      </div>
      <div>
        <label className={labelCls}>Authorized Pickups (optional)</label>
        <input type="text" value={child.authorizedPickups} onChange={(e) => onChange({ ...child, authorizedPickups: e.target.value })} placeholder="e.g. John Smith, Mary Jones" className={inputCls} style={{ fontSize: "1rem" }} />
