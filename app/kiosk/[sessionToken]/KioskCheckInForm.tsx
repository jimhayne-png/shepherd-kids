...
function RoomSelect({ value, onChange, rooms }: { value: string; onChange: (v: string) => void; rooms: Room[] }) {
  if (rooms.length === 0) return null;
  return (
    <select
      value={value}
      onChange={e => onChange(e.target.value)}
      style={{ fontSize: 16, padding: "12px 14px", borderRadius: 12, border: "2px solid #e5e7eb", backgroundColor: "white", boxSizing: "border-box" as const, width: "100%" }}
    >
...
export default function KioskCheckInForm({ sessionToken, serviceName, serviceDate, rooms }: Props) {
  ...
  const [newFamilyChildren, setNewFamilyChildren] = useState<NewChildForm[]>([emptyNewChild()]);
  ...
  async function handleLookup() {
    ...
    if (!res.ok) { setLookupError(data.error ?? "Lookup failed. Please try again."); return; }

    if (data.found) {
      setFamily(data.family);
      setExistingChildren((data.children as LookupChild[]).map(c => ({ ...c, selected: true, roomId: "" })));
      setAddedChildren([]);
      setStep("returning");
    } else {
      setParentName("");
  ...
  // ── Returning family submit ───────────────────────────────────────────────

  async function handleReturningSubmit() {
    if (!family) return;
    const selected = existingChildren.filter(c => c.selected);
    const additions = addedChildren.filter(c => c.firstName.trim() && c.lastName.trim());
    ...
    if (!res.ok) { setSubmitError(data.error ?? "Check-in failed. Please try again."); return; }
  ...
  async function handleNewFamilySubmit() {
    const validChildren = newFamilyChildren.filter(c => c.firstName.trim() && c.lastName.trim());
    if (!parentName.trim() || phone.replace(/\D/g, "").length < 7 || validChildren.length === 0) return;
  ...
  const header = (title: string, subtitle?: string) => (
    <div style={{ backgroundColor: ACCENT, padding: "24px 32px", flexShrink: 0 }}>
      <p style={{ color: "white", fontWeight: 700, fontSize: 20, margin: 0 }}>{title}</p>
      {subtitle && <p style={{ color: "white", opacity: 0.75, fontSize: 14, margin: "4px 0 0" }}>{subtitle}</p>}
    </div>
  );
  ...
  if (step === "success") return (
    <div style={{ minHeight: "100dvh", backgroundColor: "#f9fafb", display: "flex", flexDirection: "column" }}>
      <div style={{ backgroundColor: "#16a34a", padding: "24px 32px" }}>
        ...
        <p style={{ color: "white", opacity: 0.75, fontSize: 14, margin: "4px 0 0" }}>{serviceSubtitle}</p>
      </div>
      <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "40px 32px" }}>
        <div style={{ width: "100%", maxWidth: 480, textAlign: "center" }}>
          ...
          <div style={{ backgroundColor: "#f0fdf4", border: "3px solid #22c55e", borderRadius: 20, padding: "32px 24px", marginBottom: 32 }}>
            ...
            <p style={{ fontSize: 14, color: "#6b7280", marginTop: 12, marginBottom: 0 }}>Show this code at pickup</p>
          </div>
          <button onClick={reset} style={{ width: "100%", padding: "20px", borderRadius: 20, border: "none", backgroundColor: ACCENT, color: "white", fontSize: 20, fontWeight: 800, cursor: "pointer" }}>
            Done (Start Over)
          </button>
        </div>
      </div>
  ...
  if (step === "returning" && family) {
    ...
    return (
      <div style={{ minHeight: "100dvh", backgroundColor: "#f9fafb", display: "flex", flexDirection: "column" }}>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "32px", maxWidth: 600, margin: "0 auto", width: "100%", boxSizing: "border-box" as const }}>
          <h2 style={{ fontSize: 28, fontWeight: 800, color: "#111827", marginBottom: 4 }}>
            {family.parentFirstName} {family.parentLastName}
          </h2>
          <p style={{ fontSize: 16, color: "#6b7280", marginBottom: 24 }}>Select children to check in today:</p>
          ...
          {existingChildren.map((child, i) => (
            <div key={child.id} style={{ marginBottom: 12 }}>
              <button
                type="button"
                onClick={() => setExistingChildren(cs => cs.map((c, j) => j === i ? { ...c, selected: !c.selected } : c))}
                style={{
              ...
              >
                <div style={{
                  ...
                  {child.selected && <span style={{ color: "white", fontWeight: 900, fontSize: 16 }}>✓</span>}
                </div>
                <span style={{ fontSize: 20, fontWeight: 700, color: "#111827" }}>{child.name}</span>
              </button>
          ...
          {submitError && <p style={{ color: "#dc2626", textAlign: "center", marginBottom: 12, fontSize: 15 }}>{submitError}</p>}
          ...
          <button
            ...
            style={{
              ...
              fontSize: 22, fontWeight: 800, cursor: anyReady && !submitting ? "pointer" : "default",
              marginBottom: 12,
            }}
          >
  ...
  if (step === "new") {
    const validCount = newFamilyChildren.filter(c => c.firstName.trim() && c.lastName.trim()).length;
    const canSubmit = !!(parentName.trim() && phone.replace(/\D/g, "").length >= 7 && validCount > 0);

    return (
      <div style={{ minHeight: "100dvh", backgroundColor: "#f9fafb", display: "flex", flexDirection: "column" }}>
        {header("New Family Registration", serviceSubtitle)}
        <div style={{ flex: 1, overflowY: "auto", padding: "32px", maxWidth: 600, margin: "0 auto", width: "100%", boxSizing: "border-box" as const }}>
          <h2 style={{ fontSize: 26, fontWeight: 800, color: "#111827", marginBottom: 20 }}>Parent / Guardian Info</h2>

          <div style={{ marginBottom: 16 }}>
            <label style={{ display: "block", fontSize: 15, fontWeight: 700, color: "#374151", marginBottom: 8 }}>Full Name *</label>
            <input
              value={parentName}
          ...
          <div style={{ marginBottom: 28 }}>
            <label style={{ display: "block", fontSize: 15, fontWeight: 700, color: "#374151", marginBottom: 8 }}>Phone *</label>
            <input
              type="tel"
              inputMode="numeric"
              value={phone}
          ...
          <h3 style={{ fontSize: 22, fontWeight: 800, color: "#111827", marginBottom: 16 }}>Children</h3>
          ...
          {newFamilyChildren.map((child, i) => (
            <NewChildCard
              ...
              index={i}
              rooms={rooms}
              onChange={updated => setNewFamilyChildren(cs => cs.map((c, j) => j === i ? updated : c))}
              onRemove={newFamilyChildren.length > 1 ? () => setNewFamilyChildren(cs => cs.filter((_, j) => j !== i)) : undefined}
            />
          ))}
          ...
          <button
            ...
            style={{
              ...
              fontSize: 22, fontWeight: 800, cursor: canSubmit && !submitting ? "pointer" : "default",
              marginBottom: 12,
            }}
          >
            {submitting ? "Checking in…" : "Check In →"}
...
function NewChildCard({
  child, index, rooms, onChange, onRemove,
}: {
  child: NewChildForm;
  index: number;
  rooms: Room[];
  ...
  return (
    <div style={{ backgroundColor: "white", border: "2px solid #e5e7eb", borderRadius: 16, padding: "16px 20px", marginBottom: 12 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
        <span style={{ fontSize: 16, fontWeight: 700, color: "#374151" }}>Child {index + 1}</span>
        {onRemove && (
          <button type="button" onClick={onRemove} style={{ color: "#dc2626", fontWeight: 700, background: "none", border: "none", fontSize: 14, cursor: "pointer" }}>
            Remove
          </button>
        )}
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 10 }}>
        <input
          ...
          style={{ fontSize: 18, padding: "12px 14px", borderRadius: 12, border: "2px solid #e5e7eb", boxSizing: "border-box" as const, outline: "none" }}
        />
        <input
      ...
      <div style={{ display: "grid", gridTemplateColumns: rooms.length > 0 ? "1fr 1fr" : "1fr", gap: 10 }}>
        <input
          type="number"
          inputMode="numeric"
          min={0}
        ...
        />
        {rooms.length > 0 && (
          <RoomSelect value={child.roomId} onChange={v => onChange({ ...child, roomId: v })} rooms={rooms} />
        )}
      </div>
    </div>
  );
